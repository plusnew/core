...
const localFactory = () =>
  store(list, (state, newValue: { key: number; value: string }) => [
    newValue,
    ...state,
  ]);
...
describe("rendering nested components", () => {
  ...
  it("does a initial list work, with pushing values with placeholder", () => {
    ...
    const Component = component("Component", () => (
      <ul>
        <local.Observer>
          {(local) => local.map((item) => <li key={item.key}>{item.value}</li>)}
        </local.Observer>
      </ul>
    ));
  ...
  it("does a initial empty list work, and updating it", () => {
    const local = store(
      [] as typeof list,
      (_state, action: typeof list) => action
    );
  ...
  it("rerendering with different order and inserted elements", () => {
    ...
    newList.forEach((newListItem, newListIndex) => {
      ...
      expect((ul.childNodes[newListIndex] as HTMLElement).innerHTML).toBe(
        newListItem.value
      );
    });
  });
  ...
  it("rerendering with different order and inserted elements", () => {
    ...
    const MainComponent = component("Component", () => (
      <span>
        <local.Observer>
          {(local) =>
            local.map((item) => (
              <PartialComponent key={item.key} value={item.value} />
            ))
          }
        </local.Observer>
      </span>
    ...
    newList.forEach((newListItem, newListIndex) => {
      ...
      expect((span.childNodes[newListIndex] as Text).textContent).toBe(
        newListItem.value
      );
    });
  });
  ...
  it("rerendering with different order and inserted elements", () => {
    ...
    const PartialComponent = component(
      ...
      (Props: Props<{ value: string }>) =>
        [
          <span key={0}>
            <Props>{(props) => props.value}</Props>0
          </span>,
          <div key={1}>
            <Props>{(props) => props.value}</Props>1
          </div>,
        ] as any
    );
  ...
  it("updating should insert correct next to siblings", () => {
    const local = store(
      [] as typeof list,
      (_previousState, action: typeof list) => action
    );

    const PartialComponent = component("Component", () => (
    ...
    const MainComponent = component("Component", () => (
      <div>
        ...
        <div>
          <local.Observer>
            {(local) =>
              local.map((item) => <div key={item.key}>{item.value}</div>)
            }
          </local.Observer>
          <PartialComponent />
        </div>
    ...
    expect((div.childNodes[2] as HTMLElement).innerHTML).toBe(
      "some other element"
    );
  });
  ...
  it("updating component with text", () => {
    const local = store(
      [{ key: 0 }, { key: 1 }],
      (_previousState, action: { key: number }[]) => action
    );

    const PartialComponent = component(
      "Component",
      (Props: Props<{ key: number }>) => (
        <Props>{(props) => `element${props.key}`}</Props>
      )
    );
    ...
    const MainComponent = component("Component", () => (
      <div>
        <local.Observer>
          {(local) => local.map((item) => <PartialComponent key={item.key} />)}
        </local.Observer>
      </div>
    ));
  ...
  it("moving component with boolean", () => {
    ...
    const PartialComponent = component(
      "Component",
      (Props: Props<{ key: number }>) => (
        <Props>{(props) => (props.key === 0 ? "foo" : false)}</Props>
      )
    );
  ...
  it("not moving when element got removed", () => {
    ...
    const local = store(
      list,
      (_state, newValues: { key: number; value: string }[]) => newValues
    );
    ...
    const Component = component(
      ...
      () => (
        <ul>
          <local.Observer>
            {(local) =>
              local.map((item) => <li key={item.key}>{item.value}</li>)
            }
          </local.Observer>
        </ul>
      )
  ...
  it("removed without key property", () => {
    ...
    const Component = component("Component", () => (
      <ul>
        <local.Observer>
          {(local) => local.map((item) => <li>{item.value}</li>)}
        </local.Observer>
      </ul>
    ));
  ...
  it("ordering with empty elements in between", () => {
    const NestedComponent = component("NestedComponent", () => false as any);
    ...
    const MainComponent = component("MainComponent", () => (
      <ul>
        <local.Observer>{(local) => local.map((item) => item)}</local.Observer>
      </ul>
    ));
  ...
  it("ordering with empty elements on end", () => {
    ...
    local.dispatch([<span key={0} />, <NestedComponent key={1} />]);
  ...
  it("ordering with empty elements on beginning", () => {
    ...
    local.dispatch([<NestedComponent key={1} />, <span key={0} />]);
  ...
  it("ordering with text in between", () => {
    const NestedComponent = component("NestedComponent", () => "foo" as any);
