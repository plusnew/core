...
import type FragmentInstance from "../../../src/instances/types/Fragment/Instance";
...
describe("rendering nested components", () => {
  ...
  it("checks if nesting the components works", () => {
    const NestedComponent = component(
      ...
      (Props: Props<{ value: string }>) => (
        <Props>{(props) => <div class={props.value}>{props.value}</div>}</Props>
      )
    );
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>
        {(local) => <NestedComponent value={local} />}
      </local.Observer>
    ));
    ...
    const NestedComponent = component(
      ...
      (Props: Props<{ value: string }>) => (
        <>
          ...
          <nestedStore.Observer>
            {(state) => <span>{state}</span>}
          </nestedStore.Observer>
        </>
      )
    );
    const MainComponent = component("Component", (_Props: Props<{}>) => (
      <mainStore.Observer>
        {(state) => <NestedComponent value={state} />}
      </mainStore.Observer>
    ));
  ...
  it("unregister dependencies", () => {
    ...
    const NestedComponent = component("Component", (_Props: Props<{}>) => (
      <span>
        <counterStore.Observer>{nestedUpdateSpy}</counterStore.Observer>
      </span>
    ));
    ...
    const MainComponent = component("Component", (_Props: Props<{}>) => (
      <>
        <span>
          <counterStore.Observer>{containerUpdateSpy}</counterStore.Observer>
        </span>
        <mainStore.Observer>
          {(state) => state && <NestedComponent />}
        </mainStore.Observer>
      </>
    ));
  ...
  it("unregister dependencies recusively when nested in array", () => {
    ...
    const MainComponent = component("Component", (_Props: Props<{}>) => (
      <>
        ...
        <mainStore.Observer>
          {(state) => state && [<NestedComponent key="0" />]}
        </mainStore.Observer>
      </>
    ));
  ...
  it("nested component should not be created when shallow mode is active", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>
        {(state) => (
          <>
            {state < 1 && <NestedComponent foo={state} />}
            <span />
          </>
        )}
      </local.Observer>
  ...
  it("nested component should not be created when shallow mode is active", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>
        {(state) =>
          state < 1
            ? [<span key={1} />, <NestedComponent key={0} foo={state} />]
            : [<NestedComponent key={0} foo={state} />, <span key={1} />]
        }
      </local.Observer>
    ));
  ...
  describe("nested render call", () => {
    ...
    it("nested component should rerender when an new object occures", () => {
      ...
      const MainComponent = component("Component", () => (
        <local.Observer>
          {(_state) => <NestedComponent>{{}}</NestedComponent>}
        </local.Observer>
      ));
  ...
  it("removed nested component gets a componentWillUnmount call", () => {
    ...
    const NestedComponent = component("Component", () => <div />);

    NestedComponent.prototype.componentWillUnmount = componentWillUnmountSpy;

    const MainComponent = component("Component", () => (
      <local.Observer>
        {(state) => (state === true ? <NestedComponent /> : null)}
      </local.Observer>
    ));

    const mainInstance = plusnew.render(<MainComponent />, {
      driver: driver(container),
    });
  ...
  it("removed nested component gets a componentWillUnmount call with props", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>
        {(state) => (state === true ? <NestedComponent foo="bar" /> : null)}
      </local.Observer>
    ));
  ...
  it("displayName is set", () => {
    const MainComponent = component("Component", () => <div />);
