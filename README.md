...
vood.controllerHelper.create('main/app' {
  filter: function(data) {
    ...
    // sets all entities inside the array *items* with matching expression, the property show to true;
    this.set('items.propertyName==' + data.value +'.show', true);
    // you dont have to write it inside keyvalue, you can use the *query* property, even with an array for an *Logic-OR*
