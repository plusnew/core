  ...
  if (isPlusnewElement(a) && isPlusnewElement(b) && a.type === b.type) {
    ...
    if (keys.length === Object.keys(b.props).length) {
      for (let i = 0; i < keys.length && isSame === true; i += 1) {
        ...
        isSame = currentKey in b && isEqual(a.props[currentKey], b.props[currentKey]);
      }
    } else {
      isSame = false;
    }
    isSame === isEqual(a.props, b.props);
    ...
    if (isPlusnewElement(a[0]) || typeof a[0] !== 'object') {
      for (let i = 0; i < a.length && isSame === true; i += 1) {
        isSame = isEqual(a[0], b[0]);
      }
    }
  } else {
    isSame = a === b; // in case it has no special condition, do a reference check
  }
