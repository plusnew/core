...
export default function <HostElement, HostTextElement>(
  ...
  instance: FragmentInstance<HostElement, HostTextElement>
) {
  for (let i = 0; i < newAbstractElement.props.children.length; i += 1) {
    if (i < instance.rendered.length) {
      const newInstance = reconciler.update(
        newAbstractElement.props.children[i],
        instance.rendered[i]
      );
  ...
  while (instance.rendered.length > newAbstractElement.props.children.length) {
    instance.rendered[newAbstractElement.props.children.length].remove(false);
